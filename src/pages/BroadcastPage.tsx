...
export default function BroadcastPage() {
  const { accounts, isRealElectron, sendCommentToFriends, broadcastProgress, broadcastControlState, pauseBroadcast, resumeBroadcast, stopBroadcast, getFriendsList } = useAppContext();
  ...
  const [loadingFriends, setLoadingFriends] = useState<Record<string, boolean>>({});
  ...
  const loadFriends = async (accountId: string) => {
    setLoadingFriends((prev) => ({ ...prev, [accountId]: true }));
    try {
    ...
    setLoadingFriends((prev) => ({ ...prev, [accountId]: false }));
  };

  const toggleFriend = (accountId: string, steamId: string) => {
    setSelectedFriends((prev) => {
      const current = prev[accountId] || [];
      const newSelection = current.includes(steamId)
        ? current.filter((id) => id !== steamId)
        : [...current, steamId];
  ...
  const handleBroadcast = async () => {
    ...
    if (selectedAccounts.length === 0) { alert("Выберите аккаунты!"); return; }
    ...
    for (const accountId of selectedAccounts) {
      try {
        ...
        const targetSteamIds = friends.length > 0 ? friends : null;
        
        const result = await sendCommentToFriends(accountId, message, delay, targetSteamIds);
  ...
  const handlePause = async (accountId: string) => {
    await pauseBroadcast(accountId);
  };

  const handleResume = async (accountId: string) => {
    await resumeBroadcast(accountId);
  };

  const handleStop = async (accountId: string) => {
    await stopBroadcast(accountId);
  };

  const totalFriends = selectedAccounts.reduce((sum, id) => {
    const friends = selectedFriends[id] || [];
    return sum + (friends.length > 0 ? friends.length : (accounts.find((a) => a.id === id)?.friendsCount || 0));
  }, 0);
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Рассылка сообщений</h1>
        <p className="text-steam-text-muted text-sm mt-1">Отправка сообщений выбранным друзьям (максимум 2 аккаунта)</p>
      </div>

      {/* Account selection with friends */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-steam-card border border-steam-border rounded-xl p-5">
        <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
          <i className="fas fa-user-circle text-steam-accent" />Выберите аккаунты и друзей (максимум 2)
        </h3>
        {onlineAccounts.length === 0 ? (
          ...
          <div className="space-y-3">
            {onlineAccounts.map((account) => {
              ...
              const canSelect = selectedAccounts.length < 2 || isSelected;
                ...
                <div key={account.id} className="border border-steam-border rounded-lg overflow-hidden">
                  <div
                    className={`flex items-center justify-between p-3 cursor-pointer transition-colors ${
                      isSelected ? "bg-steam-accent/10" : "bg-steam-darker hover:bg-steam-card-hover"
                    }`}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      {isSelected && friends.length > 0 && (
                        <span className="text-steam-accent text-xs">{selectedCount}/{friends.length} выбрано</span>
                      )}
                  ...
                  <AnimatePresence>
                    {isSelected && friends.length > 0 && (
                      <motion.div
                        ...
                        className="border-t border-steam-border overflow-hidden"
                      >
                        <div className="p-3 space-y-2">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={(e) => { e.stopPropagation(); selectAllFriends(account.id); }}
                                className="text-steam-accent text-xs hover:underline"
                              >
                                Выбрать всех
                              </button>
                              <button
                                onClick={(e) => { e.stopPropagation(); deselectAllFriends(account.id); }}
                                className="text-steam-text-muted text-xs hover:underline"
                              >
                                Снять выделение
                              </button>
                            </div>
                          ...
                          <div className="max-h-48 overflow-y-auto space-y-1">
                            {friends.map((friend) => {
                              ...
                              return (
                                <div
                                  key={friend.steamId}
                                  onClick={(e) => { e.stopPropagation(); toggleFriend(account.id, friend.steamId); }}
                                  className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${
                                    isFriendSelected
                                      ? "bg-steam-accent/15 border border-steam-accent/30"
                                      : "bg-steam-darker border border-transparent hover:border-steam-border"
                                  }`}
                                >
                                  <input
                                    type="checkbox"
                                    ...
                                    onChange={() => {}}
                                    className="w-3.5 h-3.5 rounded border-steam-border bg-steam-card text-steam-accent focus:ring-steam-accent"
                                  />
                  ...
                  {/* Loading state */}
                  {isSelected && loadingFriends[account.id] && (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-steam-card border border-steam-border rounded-xl p-5">
        <label className="text-white text-sm font-medium mb-2 block flex items-center gap-2">
          <i className="fas fa-comment-dots text-steam-accent" />Текст сообщения
        </label>
        <textarea
          value={message}
        ...
        />
        <p className="text-steam-text-muted text-xs mt-2">{message.length} символов</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-steam-card border border-steam-border rounded-xl p-5">
        <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
          <i className="fas fa-clock text-steam-yellow" />Задержка между сообщениями
        </h3>
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-20 px-3 py-1.5 bg-steam-darker border border-steam-border rounded-lg text-white text-sm text-center focus:outline-none focus:border-steam-accent"
              />
          ...
          <div className="p-3 bg-steam-yellow/10 border border-steam-yellow/30 rounded-lg">
            <p className="text-steam-yellow text-xs flex items-start gap-2">
              ...
              <span><strong>Минимальная задержка: {MIN_DELAY} секунд.</strong> Нельзя уменьшить ниже минимума.</span>
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-steam-card border border-steam-border rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
          <div className="text-center p-3 bg-steam-darker rounded-lg">
            <p className="text-steam-text-muted text-xs">Аккаунтов</p>
            <p className="text-white font-bold text-lg">{selectedAccounts.length}</p>
          </div>
          <div className="text-center p-3 bg-steam-darker rounded-lg">
        ...
        <button
          ...
          disabled={isRunning || !message.trim() || selectedAccounts.length === 0}
        ...
        >
          {isRunning ? (
            <><div className="w-5 h-5 rounded-full border-2 border-white border-t-transparent animate-spin" />Рассылка...</>
          ) : (
            <><i className="fas fa-paper-plane" />Начать рассылку</>
          )}
        </button>
      </motion.div>
      ...
      {broadcastProgress && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-steam-card border border-steam-accent/30 rounded-xl p-5">
          <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
            <i className="fas fa-tasks text-steam-accent" />Прогресс рассылки
          </h3>
          <div className="space-y-3">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white text-sm font-medium">
                    {accounts.find((a) => a.id === broadcastProgress.accountId)?.name}
                  </span>
                </div>
                <div className="flex items-center gap-3 text-xs">
                  <span className="text-steam-green"><i className="fas fa-check mr-1" />{broadcastProgress.success}</span>
                  {broadcastProgress.failed > 0 && <span className="text-steam-red"><i className="fas fa-times mr-1" />{broadcastProgress.failed}</span>}
                  <span className="text-steam-text-muted">{broadcastProgress.current}/{broadcastProgress.total}</span>
                </div>
              </div>
              ...
              {isRunning && (
                <div className="flex gap-2">
                  {broadcastControlState.isPaused ? (
                    <button
                      onClick={() => handleResume(broadcastProgress.accountId)}
                    ...
                    <button
                      onClick={() => handlePause(broadcastProgress.accountId)}
                  ...
                  <button
                    onClick={() => handleStop(broadcastProgress.accountId)}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-steam-accent/10 border border-steam-accent/30 rounded-xl p-4">
        <p className="text-steam-accent text-xs flex items-start gap-2">
          ...
          <span><strong>Важно:</strong> Сообщения будут отправлены выбранным друзьям с указанной задержкой. Рекомендуется использовать задержку не менее 10-15 секунд.</span>
        </p>
      </motion.div>
    </div>
  );
